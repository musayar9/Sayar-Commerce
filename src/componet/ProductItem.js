    ...
    <>
      <div className='p-2  border border-gray-300 rounded-xl  mb-2 lg:mb-5 w-[48%] md:w-[32%] lg:w-[23%]  space-x-2 md:space-x-0  '>
       <h5 className='p-2   font-bold text-sm md:text-md md:mb-[6px] lg:mb-[10px] products truncate '>{product.title}</h5>
        ...
        {
          !like ? <button className=' z-10 mt-2 w-8 h-8 md:w-10 md:h-10  top-2 right-8 lg:top-4 lg:right-6 border 
          ...
          : <button className='float-right  z-10 mt-2 w-8 h-8 md:w-10 md:h-10  top-2 right-8 lg:top-4 lg:right-6 border  border-gray-200  items-center justify-center flex absolute rounded-full drop-shadow-xl ' onClick={favouriteTry}><BsFillSuitHeartFill size={20} className='text-pink-700'/></button>
