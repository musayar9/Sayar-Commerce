    ...
    <>
     ...
     <picture className='relative mt-[5px]  shrink-0'>
     {
       ...
       border border-gray-300 hover:text-white
        ...
        justify-center flex absolute  rounded-full drop-shadow-xl' onClick ={starFavouriteProduct}><BsSuitHeart size={24}/></button> 
       : <button className='z-10 -mt-2 w-10 h-10 top-2 right-2 border border-gray-200  items-center justify-center flex absolute rounded-full drop-shadow-xl ' onClick={starFavouriteTry}><BsFillSuitHeartFill size={24} className='text-pink-700'/></button>
       
     } 
      
     <img src={star.image} alt={star.alt} title={star.alt} className="w-[80%] shrink-0   p-2 hover:w-[100%] duration-500 "/>
     </picture>
     ...
     
     {
       star.price > 17000 ?     <ul className='items-center justify-between text-violet-700 shrink-0'>
          <li className='line-through text-sm '> {new Intl.NumberFormat('tr-TR',{style:'currency', currency:'TRY'}).format(star.price)} </li>
     ...
     <div className='flex items-center justify-center'> 
      <p className="border w-12 h-12 border-gray-300 products flex items-center
        justify-center rounded-full text-center text-xl font-bold">{starBasketProduct ? starBasketProduct.amount : 0} </p></div>
      
         <div className='space-y-2 w-full '>
     
         <button className='px-6 py-2 w-full text-lg text-purple-600 font-semibold  rounded-full border border-purple-200 hover:text-white hover:bg-purple-600 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-offset-2
         ...
         <button className='px-6 py-2 w-full text-lg text-pink-600 font-semibold 
