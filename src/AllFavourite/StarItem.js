...
function StarItem({star}) {
    ...
    const {cardMoney, starBasket, setStarBasket, starFavourite, setStarFavourite, starMoney,setEmptyFavourite } = useSite()
  ...
  return (
    <>
      ...
      <h5 className='p-2  font-bold text-sm md:text-md mb-[4px] md:mb-[6px] lg:mb-[10px] products truncate '>{star.title}</h5>
     
     <picture className='relative mt-2 md:mt-[3px] lg:mt-[5px]  shrink-0 cursor-pointer'>
     {
       !like ? <button className='z-10 -mt-2 w-8 h-8 md:w-10 md:h-10  top-2 right-2 
       border border-gray-300 hover:text-white
        ...
        justify-center flex absolute  rounded-full drop-shadow-xl' onClick ={starFavouriteProduct}><BsSuitHeart size={20}/></button> 
       : <button className='z-10 -mt-2 w-8 h-8 md:w-10 md:h-10  top-2 right-2  border border-gray-200  items-center justify-center flex absolute rounded-full drop-shadow-xl ' onClick={starFavouriteTry}><BsFillSuitHeartFill size={20} className='text-pink-700'/></button>
       
     } 
      
     <img src={star.image} alt={star.alt} title={star.alt} className="w-[80%] shrink-0   ml-2 hover:w-[100%] duration-500 "/>
     </picture>
     ...
     
     {
       ...
       :
      <p className= 'text-sm md:text-md lg:text-lg text-violet-700 mt-4'>{new Intl.NumberFormat('tr-TR',{style:'currency', currency:'TRY'}).format(star.price)}</p>
     }
     </div>
     ...
     <div className='flex items-center justify-center'> 
      <p className="border  w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 border-gray-300 products flex items-center
        justify-center rounded-full text-center  text-sm lg:text-md font-bold">{starBasketProduct ? starBasketProduct.amount : 0} </p></div>
